...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if (empty($first_name) || empty($last_name) || empty($email) || empty($gender) || empty($date_of_birth) || empty($blood_group) || empty($address) || empty($degree) || empty($specialist) || empty($password)) {
        ...
        if (mysqli_query($conn, $sql)) {
            $dsql = "INSERT INTO userinfo (FirstName, LastName, gender,  DOB, Address, email, password, usertype, picture) 
            VALUES ('$first_name', '$last_name', '$gender',  '$dob', '$address', '$email', '$password', 'Doctor', '$picture')";
